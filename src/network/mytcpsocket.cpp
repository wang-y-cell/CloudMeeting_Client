...
MyTcpSocket::MyTcpSocket(QObject *par):QThread(par) {
    ...
    sendbuf =(std::uint8_t *) malloc(4 * MB);
	recvbuf = (std::uint8_t*)malloc(4 * MB);
    hasrecvive = 0;
}
...
void MyTcpSocket::sendData(MESG* send) {
	...
	//消息类型
	qToBigEndian<std::uint16_t>(send->msg_type, sendbuf + bytestowrite);
	bytestowrite += 2;

	//发送者ip
	std::uint32_t ip = _socktcp->localAddress().toIPv4Address();
	qToBigEndian<std::uint32_t>(ip, sendbuf + bytestowrite);
	bytestowrite += 4;
	...
	{
		//发送数据大小
		qToBigEndian<std::uint32_t>(send->len, sendbuf + bytestowrite);
		bytestowrite += 4;
		...
		qToBigEndian<std::uint32_t>(room, send->data);
	}
	...
	//----------------write to server-------------------------
	std::int64_t hastowrite = bytestowrite;
	std::int64_t ret = 0, haswrite = 0;
...
std::int64_t MyTcpSocket::readn(char * buf, std::uint64_t maxsize, int n) {
    std::uint64_t hastoread = n;
    std::uint64_t hasread = 0;
    do
    {
        std::int64_t ret  = _socktcp->read(buf + hasread, hastoread);
        if(ret < 0)
        {
            return -1;
        }
...
void MyTcpSocket::recvFromSocket() {
    ...
    */
	std::int64_t availbytes = _socktcp->bytesAvailable(); // 获取可读取的字节数
	if (availbytes <=0) { // 如果可读取的字节数小于等于0，则返回
		return;
	}
    std::int64_t ret = _socktcp->read((char *) recvbuf + hasrecvive, availbytes);
    if (ret <= 0) {
    ...
    if (hasrecvive < MSG_HEADER){ // 如果数据包不够，则返回
         ...
         * MinGW 下曾出现 uint16 写入 &type 时覆盖相邻 std::uint32_t（长度），表现为「uint16 前后 hdrLen 从 4 变 0」。 */
        const std::uint32_t nBody = qFromBigEndian<std::uint32_t>(recvbuf + 7);
		LOG_INFO("MyTcpSocket", "nBody(bytes): " << nBody);
        if ((std::uint64_t)nBody + 1 + MSG_HEADER <= hasrecvive) //收够一个包 +1 表示 '#'
        {
            if (recvbuf[0] == '$' && recvbuf[MSG_HEADER + nBody] == '#') //且包结构正确 $ + 消息类型 + 数据大小 + 数据 + #
            {
				const std::uint16_t rawKind = qFromBigEndian<std::uint16_t>(recvbuf + 1);
				...
				if (msgtype == CREATE_MEETING_RESPONSE || 
					...
					if (msgtype == CREATE_MEETING_RESPONSE) //客户端收到了服务端创建会议的响应
					{
						...
						if (nBody >= 4u) {
							roomNo = qFromBigEndian<std::int32_t>(recvbuf + MSG_HEADER);
						...
						if (msg == NULL) {
							...
							if (nBody == 0u) {
								...
								msg->data = (std::uint8_t*)malloc((std::uint64_t)nBody);
								if (msg->data == nullptr) {
									free(msg);
									LOG_ERROR("MyTcpSocket", "CREATE_MEETING_RESPONSE malloc MESG.data failed");
								} else {
									memset(msg->data, 0, (std::uint64_t)nBody);
									/* 与 Widget::datasolve 中 memcpy(&roomno, msg->data, msg->len) 一致：写入主机字节序房间号 */
									if (nBody >= 4u)
										memcpy(msg->data, &roomNo, sizeof(std::int32_t));
									if (nBody > 4u)
										memcpy(msg->data + sizeof(std::int32_t),
											   recvbuf + MSG_HEADER + sizeof(std::int32_t),
											   (size_t)nBody - sizeof(std::int32_t));
									LOG_INFO("MyTcpSocket", "将消息体放入接收队列: queue_recv.push_msg(msg)");
									queue_recv.push_msg(msg);
								}
							}
						...
						if (msg == NULL) {
							...
							memset(msg, 0, sizeof(MESG));
							msg->msg_type = msgtype;
							msg->data = (std::uint8_t*)malloc(nBody);
							if (msg->data == NULL) {
								free(msg);
					...
					const std::uint32_t ip = qFromBigEndian<std::uint32_t>(recvbuf + 3);
					...
					if (msgtype == IMG_RECV) {
						...
						if (rdc.size() > 0)
						{
							...
							else
							{
								memset(msg, 0, sizeof(MESG));
								msg->msg_type = msgtype;
								msg->data = (std::uint8_t*)malloc(rdc.size()); // 10 = format + width + width
								if (msg->data == NULL)
								{
									free(msg);
					...
					else if (msgtype == AUDIO_RECV)
					{
						...
						if (rdc.size() > 0) {
							...
							if (msg == NULL) {
								...
								msg->data = (std::uint8_t*)malloc(rdc.size());
								if (msg->data == nullptr)
								{
                                    free(msg);
                        ...
                        if(decoded.size() > 0) {
                            ...
                            if (msg == NULL) {
                                ...
                                msg->ip = ip;
                                msg->data = (std::uint8_t*)malloc(decoded.size());
                                if (msg->data == nullptr) {
                                    free(msg);
			...
			memmove_s(recvbuf, 4 * MB, recvbuf + MSG_HEADER + nBody + 1, hasrecvive - ((std::uint64_t)nBody + 1 + MSG_HEADER));
			hasrecvive -= ((std::uint64_t)nBody + 1 + MSG_HEADER);
        }
        else
        {
            return;
